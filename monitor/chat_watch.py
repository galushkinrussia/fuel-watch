...
SYSTEM_PROMPT = (
    ...
    "с топливом на АЗС за последний час. Составь КРАТКУЮ ТЕЗИСНУЮ сводку на русском.\n\n"
    "ФОРМАТ: только короткие тезисы, каждый с новой строки и с «•». "
    "Без заголовков, вступлений и раздела «выводы».\n"
    "Например:\n"
    "• 92-й есть на Лукойле (Пархоменко), по меткам\n"
    "• В Кировском на Лукойле очередь «на ночевку», заправляют с 8 утра\n"
    "• Спрашивают, где залить 95 в канистры без конфликтов с очередью\n\n"
    "ПРАВИЛА:\n"
    "1. НЕ упоминай имена и ники.\n"
    "2. Формулируй тезис по сути и НЕ начинай каждую строку с «Водители» — "
    "это подразумевается.\n"
    "3. НЕ превращай вопросы в утверждения и не додумывай ответы.\n"
    "4. Только факты из сообщений; нет данных по теме — не выдумывай.\n"
    "5. Не более 15 тезисов и не длиннее 1500 символов суммарно — это ЖЁСТКОЕ "
    "ограничение. Простой текст без разметки."
)
...
def build_llm_input(messages):
    """Собирает новые сообщения в один текст для LLM — без имён авторов."""
    lines = []
    for m in messages:
        body = " ".join((m.get("body") or "").split())
        if len(body) > 300:
        ...
        if not body:
            continue
        reply = ""
        if m.get("reply_to_excerpt"):
            ex = " ".join((m.get("reply_to_excerpt") or "").split())
            if ex:
                reply = f" (в ответ на: «{ex[:60]}»)"
        lines.append(f"- {body}{reply}")
    return "\n".join(lines)
...
def summarize_with_llm(text, api_key, base_url=DEFAULT_LLM_BASE_URL,
    ...
    if len(out.encode("utf-8")) > MAX_SUMMARY_BYTES:
        shorten = (
            "Сократи список тезисов ниже до %d символов, сохранив главные тезисы. "
            "Формат — те же «•»-тезисы, без заголовков и пояснений." % MAX_SUMMARY_CHARS
        )
        try:
