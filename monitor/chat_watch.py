...
SYSTEM_PROMPT = (
    ...
    "с топливом на АЗС. Составь краткую сводку на русском языке.\n\n"
    "СТРОГИЕ ПРАВИЛА (соблюдай обязательно):\n"
    "1. НЕ превращай ВОПРОСЫ в утверждения. Если водитель спрашивает "
    "(«есть ли бензин?», «с какого времени отпускают?», «где заправиться?»), "
    "пиши именно «водители спрашивают…» и НЕ давай ответ, которого нет в "
    "переписке. Не выдумывай «на многих АЗС…», если это нигде не сказано.\n"
    "2. НЕ меняй смысл реплик на противоположный. Спросил про наличие — не пиши "
    "«подтверждает наличие». Говорит «нет бензина» — не пиши «есть».\n"
    "3. Опирайся только на сказанное. Лучше написать «не упоминается», чем "
    "додумать. Обобщения и выводы делай только если они прямо следуют из текста.\n"
    "4. Разделяй и помечай: [факт] — подтверждённое, [вопрос] — что спрашивают, "
    "[слух/мнение] — предположения водителей.\n"
    "5. Сгруппируй по темам, будь лаконичен (до 1200 символов), простой текст "
    "без разметки."
)
