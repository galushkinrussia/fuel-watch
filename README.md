...
отправляет геолокацию, задаёт радиус и топливо — ntfy-тема генерируется
автоматически. Боты Telegram доступны частным лицам без статуса.
...
/status             — мои настройки (в т.ч. тема ntfy)
/help               — помощь
```
...
Пользователь становится «активным» (получает уведомления), когда задал `lat`
и `lon` (через `/loc` или вручную). Тема ntfy создаётся автоматически при
регистрации и видна в `/status`. Монитор `fuel-monitor` обходит всех активных
пользователей и шлёт каждому push в его тему и в этот чат.
